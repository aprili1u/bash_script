...
class Network:
    def __init__(self, num_nodes, interactions_per_node, hawk_dove_payoff, memory_cost, initial_memory_poisson, initial_aggression, network_methode):
        ...
        self.aggression = np.random.normal(initial_aggression, 0.05, self.num_nodes) #initial aggression is a Gaussian(loc, 0.05)
        ...
        for i in range(self.num_nodes):
            #self.nodes.append(Node(sizes[i], memory[i], self.aggression[i]))
            self.nodes.append(Node(i/(self.num_nodes-1), memory[i], self.aggression[i])) #the size is: i/(self.num_nodes-1)
